...
message=FALSE, autodep=TRUE, fig.width=7, fig.height=5)
opts_chunk$set(warning=FALSE, tidy=FALSE, refresh=TRUE, fig.retina=2)
```
...
sigma_p <- 0.3  # process noise standard deviation
...
'
stan_ricker_missing1 <- stan_model(model_code = stan_model_missing1)

```

```{r, results="hide"}
missing_id <- sample(seq_len(N), 1)
y_missing <- y
...
```{r stan-impute3, fig.width=8, fig.height=7}
p <- extract(m_missing)
...
pch[missing_id] <- 21
col[missing_id] <- "blue"
plot(x, y, type = "o", pch = pch, ylim = c(min(y_missing), max(y_missing[-missing_id]) * 1.1), col = col, log = "y")
...
```{r stan-impute5 stan-impute4, fig.width=8, fig.height=7}
